...
const GlitchButton = () => {
  ...
  const generateRandomDivs = () => {
    ...
    const divs = [];
    for (let i = 0; i < 5; i++) {
      const width = `${Math.random() * 50 + 30}px`; // Random width between 30px and 80px
      const height = `${Math.random() * 10 + 5}px`; // Random height between 5px and 15px
  ...
  return (
    <button
      type="button"
      className="relative px-10 py-2 bg-black border border-white/30 rounded-full text-white font-semibold overflow-hidden group"
    >
      {randomDivs}
